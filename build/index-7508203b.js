import { i as Build, j as getAssetPath } from './index-8e01e03f.js';
import { _ as __extends, c as __spreadArrays, a as __assign, d as __awaiter, e as __generator } from './tslib.es6-4451ae6b.js';
...
const PLAY_BUTTON_TIP = "Click here to paly";
...
const VIDEO_WIDTH_CONFIG = {
  min: DEFAULT_VIDEO_WIDTH,
  ideal: DEFAULT_VIDEO_WIDTH,
  max: DEFAULT_VIDEO_WIDTH,
};
const VIDEO_HEIGHT_CONFIG = {
  min: DEFAULT_VIDEO_HEIGHT,
  ideal: DEFAULT_VIDEO_HEIGHT,
  max: DEFAULT_VIDEO_HEIGHT,
};
const VIDEO_FRAME_RATE_CONFIG = {
  min: DEFAULT_FRAME_RATE,
  ideal: DEFAULT_FRAME_RATE,
  max: DEFAULT_FRAME_RATE,
};
const AUDIO_ECHO_CONFIG = {
  exact: true,
};
const MEDIA_DEVICE_CONFIG = {
  audio: {
    echoCancellation: AUDIO_ECHO_CONFIG,
  },
  video: {
    width: VIDEO_WIDTH_CONFIG,
    // height: VIDEO_HEIGHT_CONFIG,
    frameRate: VIDEO_FRAME_RATE_CONFIG,
...
const FACE_API_CONFIG = {
  ...
  // default: 320
  inputSize: DEFAULT_FACE_API_INPUT_SIZE,
  // minimum confidence threshold
  // default: 0.5
  scoreThreshold: DEFAULT_FACE_API_SCORE,
};
const IMAGE_CAPTURE_SCORE_THRESHOLD = 0.9;
const VIDEO_CAPTURE_SCORE_THRESHOLD = 0.5;

let config = null;
const setApiConfig = (conf) => (config = conf);
const getDocApiConfig = () => {
  return config;
};
const getDocMetaData = (fileType) => {
  const { policyNumber, imageDocMetadata, videoDocMetadata } = getDocApiConfig();
  const defaultData = Object.assign({ doc_meta_data: "", file_type: fileType === "IMAGE" ? DEFAULT_IMAGE_DOC_TYPE : DEFAULT_VIDEO_DOC_TYPE, policy_number: policyNumber, DocumentName: getFileName(fileType) || DEFAULT_DOC_NAME, DocumentCategory: fileType === "IMAGE" ? DEFAULT_IMAGE_CATEGORY : DEFAULT_VIDEO_CATEGORY }, (fileType === "IMAGE" ? { bypass_image_conversion: "true" } : {}));
  return Object.assign(defaultData, fileType === 'IMAGE' ? imageDocMetadata : videoDocMetadata);
};
const getFileName = (fileType) => {
  const { policyNumber, imageDocMetadata = {}, videoDocMetadata = {} } = getDocApiConfig();
  if (policyNumber) {
    return fileType === "IMAGE"
      ? `${policyNumber}_${imageDocMetadata.file_type || DEFAULT_IMAGE_DOC_TYPE}.png`
      : `${policyNumber}_${videoDocMetadata.file_type || DEFAULT_VIDEO_DOC_TYPE}.mp4`;
  }
  return null;
};

var _a$1;
const hasGetUserMedia = !!((_a$1 = navigator === null || navigator === void 0 ? void 0 : navigator.mediaDevices) === null || _a$1 === void 0 ? void 0 : _a$1.getUserMedia);
const requestUserMedia = async () => {
  if (!hasGetUserMedia) {
    return { errMessage: NO_MEDIA_ACCESS };
  }
  try {
    console.log("constr", navigator.mediaDevices.getSupportedConstraints());
    const stream = await navigator.mediaDevices.getUserMedia(MEDIA_DEVICE_CONFIG);
    return { data: stream };
  }
  catch (error) {
    if ((error === null || error === void 0 ? void 0 : error.name) === "NotAllowedError") {
      return { warning: MEDIA_ACCESS_REQUEST };
    }
    return { errMessage: (error === null || error === void 0 ? void 0 : error.message) || DEFAULT_ERR };
  }
};
const releaseUserMedia = (stream) => {
  if (stream) {
...
const getMediaRecorder = (stream) => {
  try {
    ...
    return { data: recorder || null };
  }
  catch (error) {
    return { errMessage: error || MEDIA_RECORDER_ERR };
  }
};
const getAudioMeter = (stream) => {
  ...
  return () => {
    ...
    const volumeSum = volumes.reduce((sV, cV) => sV += cV, 0);
    const avgVolume = volumeSum / volumes.length;
    return avgVolume;
  };
};
...
function matchDimensions$1(input, reference, useMediaDimensions) {
...
const getFaceApiOptions = (options = FACE_API_CONFIG) => {
  return new TinyFaceDetectorOptions(options);
};
const loadFaceApiModels = async (path) => {
  try {
    await loadTinyFaceDetectorModel(path || FACE_API_MODELS_PATH);
  }
  catch (error) {
    return { errMessage: error || "Error while loading face api models" };
  }
};
const detectFace = async (videoEl, scoreThreshold) => {
  const options = getFaceApiOptions(scoreThreshold && Object.assign(Object.assign({}, FACE_API_CONFIG), { scoreThreshold }));
  try {
    const data = await detectSingleFace(videoEl, options);
...
const matchDimensions = (canvasEL, videoEl) => {
  return matchDimensions$1(canvasEL, videoEl, true);
};
...
const uploadFile = async (file, onUploadProgress) => {
  try {
    ...
    const request = {
      fileName: getFileName(fileType) || file.name,
      metaData: getDocMetaData(fileType),
    };
    const signedUrlRes = await getSignedUrl(request, file);
    if (signedUrlRes && signedUrlRes.signedUrl) {
      await initiateUpload(signedUrlRes.signedUrl, file, onUploadProgress);
      return { data: signedUrlRes };
    }
  }
  catch (error) {
    return { errMessage: error };
...
const getSignedUrl = async (request, file) => {
  try {
    const { url, headers } = getDocApiConfig();
    const result = await axios.put(url, request, {
      headers: Object.assign(Object.assign({}, headers), { "X-DS-CONTENT-TYPE": file.type }),
    });
    return result.data;
  }
  catch (error) {
...
const initiateUpload = async (signedUrl, file, onUploadProgress) => {
  try {
    await axios.create()({
      url: signedUrl,
      method: "PUT",
...
export { EColors as E, IMAGE_CAPTURE_SCORE_THRESHOLD as I, PLAY_BUTTON_TIP as P, RECORD_BUTTON_TIP as R, VIDEO_CAPTURE_SCORE_THRESHOLD as V, getMediaRecorder as a, createImageFile as b, createVideoFile as c, drawDetection as d, animateCircleDraw as e, detectFace as f, getAudioMeter as g, getImagePath as h, releaseUserMedia as i, getDataUrl as j, RETAKE_BUTTON_TIP as k, loadFaceApiModels as l, requestUserMedia as r, setApiConfig as s, uploadFile as u };
